...
| `'accent-top'` | `ToastModifier.ACCENT_TOP` | The color accent becomes a thin horizontal strip across the top of the card instead of a left vertical bar. At rest the close bar stays collapsed (the top strip already carries the color identity), but hovering/focusing a closable toast still reveals a working close button, same as the default look. Dismissal also works via click/Enter/Space/Escape on the row regardless. |
| `'close-hidden'` | `ToastModifier.CLOSE_HIDDEN` | No accent-bar/close-swatch chrome at all, ever - just text (and buttons/progress, if present). Dismissal still works via click/Enter/Space/Escape on the row, same as `accent-top` above. |
